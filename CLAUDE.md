...
| Table | Purpose |
|---|---|
| `users` | Auth: email, username, bcrypt hash, 6-digit code, `avatar` filename |
...
| `user_groups` | Groups: name, emoji, creator_id, rotation_order (JSON), rotation_idx, last_rotation_date |
| `group_members` | Group membership — `(group_id, user_id)` |
| `group_messages` | Group chat messages — `(id, group_id, user_id, text, created_at)` |
...
Friends:   GET /friends   GET /friends/requests
...
Groups:    GET  /groups                    ← lists with per-user unreadCount + advances rotation
           ...
           GET  /groups/:id/messages       ← fetches (DESC LIMIT ?) + marks read
           POST /groups/:id/messages       ← sends + marks sender read
...
**Route ordering matters:** `GET /api/groups/unread-count` and `GET /api/messages/unread-count` must be registered **before** their respective `/:id` / `/:friendId` routes.
...
**`src/components/Toast.jsx`** — `ToastProvider` + `useToast()` hook. Call `toast.show(message, type, duration?)` where `type` is `'success' | 'error' | 'info'`. Toasts stack at the top of the screen and auto-dismiss.

**`src/components/UserAvatar.jsx`** — Use everywhere instead of hand-rolled avatar divs. Accepts `{ user, size, className }`. `user.avatar` may be a full URL or just a filename — the `avatarUrl(avatar)` helper (also exported) normalises both. Falls back to initials + gradient on image load error. Also export `avatarUrl` for places that need the URL string directly.
...
- **`AppContext`** — All social state + side effects:
  ...
  - **`fetchGroupMessages(groupId)`** — calls `GET /api/groups/:id/messages`, immediately zeros local `unreadCount`, refreshes total.

### Navigation

No router library. `App.jsx` owns `view` state: `dashboard | record | reveal | messages | group | profile`. Bottom nav has 5 tabs; `RevealView` is defined inline in `App.jsx`. All other views are in `src/views/`.

Views with internal sub-screens manage their own `screen` state + `AnimatePresence`.

**`openReveal(userId?, vlogId?)`** — both arguments optional. Passing `vlogId` pre-selects that vlog in `RevealView`; `userId = null` shows the current user's own vlogs.

**`GroupView` screens:** `list | create | detail | chat | add-member`. `GroupDetail` receives `onReveal` prop to open any member's vlogs. Chat polls `GET /api/groups/:id/messages` every 5 s while open.

**`MessagesView`** — `ConversationView` polls `GET /api/messages/:friendId` every 3 s while open. Uses optimistic message insertion with rollback on error.
...
3. "Fertig" → `TitleScreen` → `ProcessingScreen`: merges all clip blobs, uploads via XHR with progress. Sends title, thumbnail (base64 first frame), duration, clipCount, emoji.
